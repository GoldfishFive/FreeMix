...
def get_freemix_segmant_dataset_dicts(
    names, filter_empty=True, min_keypoints=0, proposal_files=None, scene_name=None,
):
    """
    Load and prepare dataset dicts for instance detection/segmentation and semantic segmentation.
    ...
    assert len(names), names
    dataset_dicts = [
        wrap_metas(DatasetCatalog.get(dataset_name), dataset_name=dataset_name, scene_name=scene_name)
        for dataset_name in names
    ]
    for dataset_name, dicts in zip(names, dataset_dicts):
        assert len(dicts), "Dataset '{}' is empty!".format(dataset_name)
...
def _freemix_train_loader_from_config(cfg, dataset_name, mapper, *, dataset=None, sampler=None):
    scene_name ="nature"
    if "camvid" in dataset_name:
        scene_name = cfg.Camvid.Scene_name
    elif "potsdam" in dataset_name:
        scene_name = cfg.Potsdam.Scene_name
    elif "GID_15" in dataset_name:
...
@configurable(from_config=_freemix_train_loader_from_config)
def build_freemix_detection_train_loader(
    dataset,
    *,
    mapper,
    sampler=None,
...
def _freemix_test_loader_from_config(cfg, dataset_name, mapper=None):
    """
    Uses the given `dataset_name` argument (instead of the names in cfg), because the
    standard practice is to evaluate each test set individually (not combining them).
    """
    if isinstance(dataset_name, str):
        dataset_name = [dataset_name]
    scene_name = "nature"
    if "camvid" in dataset_name[0]:
        scene_name = cfg.Camvid.Scene_name
    elif "potsdam" in dataset_name[0]:
        scene_name = cfg.Potsdam.Scene_name
    elif "GID_15" in dataset_name:
        scene_name = cfg.GID_15.Scene_name
    elif "GID_5" in dataset_name:
...
def build_detection_test_loader(
    ...
    return data_loader
@configurable(from_config=_freemix_test_loader_from_config)
def build_freemix_detection_test_loader(
    dataset, *, mapper, sampler=None, num_workers=0, samples_per_gpu=1
):
    """
    Similar to `build_detection_train_loader`, but uses a batch size of 1,
