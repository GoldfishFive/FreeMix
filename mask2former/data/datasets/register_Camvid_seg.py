...
CLASS_NAMES = (
    ...
    'Fence','LaneMkgsDriv','LaneMkgsNonDriv','Misc_Text','MotorcycleScooter','OtherMoving','ParkingBlock',
    'Pedestrian','Road','RoadShoulder','Sidewalk','SignSymbol','Sky','SUVPickupTruck','TrafficCone','TrafficLight','Train',
    'Tree','Truck_Bus','Tunnel','VegetationMisc','Wall'
) # 31 without "Void"

BASE_CLASS_NAMES = [
    c for i, c in enumerate(CLASS_NAMES) if i not in [26, 27, 28, 29, 30] #类别名字对应的训练标签id
]
NOVEL_CLASS_NAMES = [
    c for i, c in enumerate(CLASS_NAMES) if i in [26, 27, 28, 29, 30]
]
...
def register_all_camvid(root):
    root = os.path.join(root, "Camvid_fmt-VOC")
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        base_name = f"camvid_base_sem_seg_{name}"
        ...
        novel_name = f"camvid_novel_sem_seg_{name}"
        DatasetCatalog.register(
            novel_name,
            lambda x=image_dir, y=gt_dir: load_sem_seg(
                y, x, gt_ext="png", image_ext="png"
