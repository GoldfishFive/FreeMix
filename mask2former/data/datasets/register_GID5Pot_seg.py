...
CLASS_NAMES = (
    'built-up', 'farmland', 'forest', 'meadow', 'water', 'impervious_surface', 'building', 'low_vegetation', 'tree', 'car', 'clutter'
)  # 11 classes

BASE_CLASS_NAMES = [
    c for i, c in enumerate(CLASS_NAMES) if i not in [3, 4, 7, 8, 10]  # 类别名字对应的训练标签id
]
NOVEL_CLASS_NAMES = [
    c for i, c in enumerate(CLASS_NAMES) if i in [3, 4, 7, 8, 10]
]
...
def register_all_GID5Pot(root):
    root = os.path.join(root, "GID5_Potsdam_Mix")
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        all_name = f"GID5Pot_sem_seg_{name}"
        ...
        base_name = f"GID5Pot_base_sem_seg_{name}"
        ...
        novel_name = f"GID5Pot_novel_sem_seg_{name}"
        DatasetCatalog.register(
            novel_name,
            lambda x=image_dir, y=gt_dir: load_sem_seg(
                y, x, gt_ext="png", image_ext="png"
