...
CLASS_NAMES = (
    'industrial land', 'urban residential', 'rural residential', 'traffic land', 'paddy field', 'irrigated land',
    'dry cropland', 'garden plot', 'arbor woodland', 'shrub land', 'natural grassland', 'artificial grassland', 'river',
    'lake', 'pond'
)  # 15 classes

BASE_CLASS_NAMES = [
    c for i, c in enumerate(CLASS_NAMES) if i not in [2, 6, 14]  # 类别名字对应的训练标签id
]
NOVEL_CLASS_NAMES = [
    c for i, c in enumerate(CLASS_NAMES) if i in [2, 6, 14]
]
...
def register_all_GID_15(root):
    root = os.path.join(root, "GID_15_fmt_VOC")
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        all_name = f"GID_15_sem_seg_{name}"
        ...
        base_name = f"GID_15_base_sem_seg_{name}"
        ...
        novel_name = f"GID_15_novel_sem_seg_{name}"
        DatasetCatalog.register(
            novel_name,
            lambda x=image_dir, y=gt_dir: load_sem_seg(
                y, x, gt_ext="png", image_ext="png"
