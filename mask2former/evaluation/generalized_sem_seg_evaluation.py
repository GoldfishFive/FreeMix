...
class GeneralizedSemSegEvaluator(SemSegEvaluator):
    ...
    def evaluate(self):
        ...
        * Mean uint8ersection-over-union averaged across classes (mIoU)
        ...
        * Pixel Accuracy (pACC)
        ...
        if self._output_dir:
            PathManager.mkdirs(os.path.join(self._output_dir, self._dataset_name))
            file_path = os.path.join(self._output_dir, self._dataset_name, "sem_seg_predictions.json")
        ...
        iou_valid = np.logical_and(acc_valid, union > 0) # SemSegEvaluator
        ...
        if self._output_dir:
            file_path = os.path.join(self._output_dir, self._dataset_name, "sem_seg_evaluation.pth")
