...
class GeneralizedPseudoSemSegEvaluator(SemSegEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            pred = np.array(pred.to(self._cpu_device), dtype=np.uint8)
    ...
    def evaluate(self):
        ...
        * Pixel Accuracy (pACC)
        ...
        acc = np.full(self._num_classes, np.nan, dtype=np.float_)
        iou = np.full(self._num_classes, np.nan, dtype=np.float_)
        tp = self._conf_matrix.diagonal()[:-1].astype(np.float_)
        pos_gt = np.sum(self._conf_matrix[:-1, :-1], axis=0).astype(np.float_)
        class_weights = pos_gt / np.sum(pos_gt)
        pos_pred = np.sum(self._conf_matrix[:-1, :-1], axis=1).astype(np.float_)
        acc_valid = pos_gt > 0
