...
class ClipAdapter(nn.Module):
    ...
    def _get_text_features(self, noun_list: List[str], task_name: str):
        if not self.prompt_learner.with_trainable_params: #prompt不是可学习的，每次只获取没有计算过文本特征的单词的文本特征
...
class MaskFormerClipAdapter(ClipAdapter):
    ...
    def get_text_features(self, noun_list: List[str], task_name: str):
        ...
        return torch.cat([object_text_features, non_object_text_features], dim=0) # 多一个非对象的类别特征
