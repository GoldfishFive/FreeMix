...
def batch_sigmoid_focal_loss(inputs, targets, alpha: float = 0.25, gamma: float = 2):
    ...
    # print("focal_pos", focal_pos.shape)
    # print("targets", targets.shape)
    # print("focal_neg", focal_neg.shape)
    """
    focal_pos torch.Size([128, 128])
    targets torch.Size([6, 16384])
    focal_neg torch.Size([128, 128])
    ////////
    focal_pos torch.Size([100, 16384])
    targets torch.Size([11, 16384])
    focal_neg torch.Size([100, 16384])
    
    focal_pos torch.Size([100, 16384])
    targets torch.Size([15, 16384])
    focal_neg torch.Size([100, 16384])
    
    focal_pos torch.Size([100, 16384])
    targets torch.Size([16, 16384])
    focal_neg torch.Size([100, 16384])
...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            # print("out_mask1", out_mask.shape)
            # print("tgt_mask1", tgt_mask.shape)
            # Flatten spatial dimension
            out_mask = out_mask.flatten(1)  # [batch_size * num_queries, H*W]
            tgt_mask = tgt_mask[:, 0].flatten(1)  # [num_total_targets, H*W]
            # print("out_mask2", out_mask.shape)
            # print("tgt_mask2", tgt_mask.shape)
            """
            out_mask torch.Size([100, 128, 128])
            tgt_mask torch.Size([9, 1, 128, 128])
            
            out_mask torch.Size([100, 16384])
            tgt_mask torch.Size([9, 16384])
            """
            ...
            # C  = torch.where(torch.isnan(C), torch.full_like(C , 100), C)  #将nan替换为 10000
