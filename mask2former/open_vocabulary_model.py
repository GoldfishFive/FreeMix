...
class OpenVocabulary(MaskFormer):
    ...
    #             for c in range(mask_pred_result.size(0)):
    #                 color = list(np.random.choice(range(256), size=3))
    #                 pre_map = mask_pred_result[c].cpu().numpy() * 255
    ...
    def forward(self, batched_inputs, text_labels=None):
        ...
        if self.training:
            ...
            #     sam_masks = torch.from_numpy(sam_masks).to(self.device)
            #     # print("sam_masks.shape", sam_masks.shape) # torch.Size([45, 512, 512])
            #     # print("mask_pred_result.shape", mask_pred_result.shape) # torch.Size([100, 128, 128])
            #     sam_masks = F.interpolate(sam_masks.unsqueeze(0), scale_factor=0.25, mode='nearest').squeeze(0)
            #     # print("sam_masks.shape", sam_masks.shape) #torch.Size([45, 128, 128])
            #     if new_pre_masks is None:
            #         new_pre_masks = torch.vstack([mask_pred_results[b], sam_masks[:10,:,:]]).unsqueeze(0)
            #     else:
            #         new_pre_masks = torch.cat((torch.vstack([mask_pred_results[b], sam_masks[:10, :, :]]).unsqueeze(0), new_pre_masks),dim=0)
            # outputs["pred_masks"] = new_pre_masks
            # print("new_pre_masks.shape", new_pre_masks.shape)  # torch.Size([45, 128, 128])
    ...
    def semantic_inference(self, mask_cls, mask_pred, image, class_names, task_name, dataset_name):
        ...
        if self.clip_ensemble:
            ...
            if self.clip_ensemble_weight > 0:
                ...
                mask_cls = trained_mask * torch.pow(mask_cls, self.clip_ensemble_weight) * torch.pow(map_back_clip_cls, 1 - self.clip_ensemble_weight) \
                           + (1 - trained_mask) * torch.pow(mask_cls, 1 - self.clip_ensemble_weight) * torch.pow(map_back_clip_cls, self.clip_ensemble_weight)
            else:
                mask_cls = clip_cls
